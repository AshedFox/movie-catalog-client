...
import { GetMoviesReviewsRelayDocument, SortDirectionEnum } from '@shared/api/graphql';
...
const UserMoviesReviewsList = ({ reviewsCount, height, userId }: Props) => {
  const { data } = useSuspenseQuery(GetMoviesReviewsRelayDocument, {
    variables: {
      withMovie: true,
      first: reviewsCount,
      filter: {
  ...
  const reviewsEdges = data.getMoviesReviewsRelay.edges;
    ...
    <div className="overflow-y-auto w-full" style={{ height }} ref={parentRef}>
      <div className="relative w-full" style={{ height: virtualizer.getTotalSize() }}>
        {virtualItems.map((virtualRow) => {
          console.log(reviewsEdges, virtualRow.index);
              ...
              <ReviewItem
                ...
                fullLinkSlot={
                  <Link
                    ...
                    href={`${review.movie!.__typename === 'Film' ? '/films' : '/series'}/${
                      review.movie!.id
                    }`}
                  >
                    {`See movie →`}
