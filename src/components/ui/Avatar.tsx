...
const Avatar: FC<Props> = ({ size = 'md', imageSrc, className }) => {
  return (
    <div
      className={clsx(
        'relative overflow-hidden rounded-full border border-gray-200 bg-gray-100 dark:border-gray-700 dark:bg-gray-800',
        sizeMap[size],
        className,
      )}
    >
      {imageSrc && <Image loading="lazy" src={imageSrc} alt="avatar" fill />}
    </div>
  );
};
